    ...
    private function combinedata()
    {
        ...
        for ($i=0; $i <= count($compilingdata) - 1 ; $i++) { 
            $finaldata[] = [
                ...
                "start" => $start[$i],
                "fadein"  => config('config.fade60.fadein'),
                "fadeout" => config('config.fade60.fadeout'),
                "duration" => config('config.duration60')
            ];
        }
       return $finaldata;
    }
